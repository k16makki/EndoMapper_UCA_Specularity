...
K = np.array([[3.26656113e+03, 0.00000000e+00, 1.50094794e+03],
            [0.00000000e+00, 3.24938039e+03, 2.01301123e+03],
            [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])

#K = np.array([[3.21895067e+03, 0., 1.54738621e+03],
#              [0., 3.19730708e+03,  2.08899263e+03],
#              [0., 0., 1.]])

#sequence_name = 'colon_like'
sequence_name = 'pink_paper'

#data_path = '/home/karim/Téléchargements/colonData/Blender_simulation/SpecularImages/'

data_path = '/home/karim/Téléchargements/real_pink_plane/final_ICCV_dataset/Images/'

#data_path = '/home/karim/Téléchargements/colonData/SpecularData/'+sequence_name+'/'
###################################################

#if sequence_name in ['Step_0.3', 'Step_0.4']:
#    dist_tolerence = 0.7
#elif sequence_name in ['Step_0.2']:
#    dist_tolerence = 0.5
#else:
#    dist_tolerence = 0.1

dist_tolerence = 50#0.9
###################################################
...
def robust_centroid_detection(img_path, level=0.9, sigma=3, min_area= 0, max_area=1000000, show='False' ):
...
imageSet = sorted(glob.glob(data_path + '*.jpg'), \
...
output_path = os.path.join(path1.parent.absolute(), sequence_name+'_output_normals/')
print(output_path)
...
for t in range(0,len(imageSet)): #(134, 143):
    ...
    Cx, Cy, image_gray, contours, len_contours = robust_centroid_detection(imageSet[t], level=0.9997, sigma=0, show='False')

    total_specularities += len_contours

    if display_fitting == 'True':

        plt.imshow(image_gray, cmap='hsv')
    ...
    for i in range(len(Cx)):
        ...
        isophote = contours[i]
        Swap(isophote, 0, 1)
        extended_isophote = extend_contour(isophote, scale=7)
        e_lsq = fit_ellipse_lstsq(extended_isophote)

        x0, y0 = ellipse_centroid(e_lsq)

        N = normal_estimation_direct(K, x0, y0)
        ...
        if angular_error >= 20:
            print('bad case:', prefix)
        else:
            print('good case:', prefix)

        if np.absolute(angular_error) <= 1: #np.logical_and(np.logical_and(np.absolute(angular_error) <= 0.5, ecc<=1.0),dist_centroids <= dist_tolerence):#residus <= 1e-08):

            if display_fitting == 'True':
                eqn, [Z] = plot_fitting_res(X, Y, e_lsq)
                plt.contour(X, Y, eqn, [Z])
                plt.title(prefix)
                if np.absolute(angular_error) <= 10:
            ...
            worksheet.write(i + 1, 0, x0)
            worksheet.write(i + 1, 1, y0)
...
plot_error(error, os.path.join(path1.parent.absolute(), sequence_name+'.png'), title='angular difference (direct VS ours)')
plot_error(distance_centroids, os.path.join(path1.parent.absolute(), sequence_name+'_distance_centroids.png'), title='Euclidean distance between centroids ', distance='True')
